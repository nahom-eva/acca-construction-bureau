...
export default function Agreements() {
  ...
  const canCreate = ['city_admin', 'subcity_officer', 'wereda_officer', 'customer'].includes(currentUser.role);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex flex-wrap gap-2 flex-1">
          <input
            ...
            placeholder="Search by name, number, building…"
            className="border border-gray-200 rounded-lg px-3 py-2 text-sm flex-1 min-w-48 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          />
          <select
            value={filterStatus}
        ...
        {canCreate && (
          <Button onClick={() => navigate('/agreements/new')} icon="＋">
            New Application
          </Button>
        )}
      </div>
      ...
      <p className="text-xs text-gray-400">
        Showing {filtered.length} of {visibleAgreements.length} agreements
      </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Agreement #</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Applicant / Building</th>
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tier</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Fee</th>
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Expiry</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3 font-mono text-xs text-gray-900">{agr.agreementNumber}</td>
                    ...
                    <td className="px-5 py-3"><TierBadge tier={agr.tier} /></td>
                    <td className="px-5 py-3">
